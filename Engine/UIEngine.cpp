...
void UIEngine::glutDisplayFunction()
{
    ...
    std::string disparityMode; 
    
    switch(computeDisparity->computeDisparityType) {
        case FILE_MODE: disparityMode = "FILE."; break;
        case ELAS_MODE: disparityMode = "ELAS."; break;
        case BM_MODE: disparityMode = "BM.";   break;
        case SGBM_MODE: disparityMode = "SGBM."; break;
        case SPS_MODE: disparityMode = "SPS.";  break;
        default: break;
    }
    
    if (uiEngine->freeviewActive)
	{
        sprintf(str, "n-next frame   \t a-run/stop   \t esc-exit   \t f-follow camera   \t"
                " c-colours   \t t-fusion %s   \t d-disparity mode: %s", uiEngine->intergrationActive ? "off" : "on", disparityMode.data());
	}
	else
	{
        sprintf(str, "n-next frame   \t a-run/stop   \t esc-exit   \t f-free view   \t"
                " t-fusion %s   \t w-write disk   \t d-disparity mode: %s", uiEngine->intergrationActive ? "off" : "on", disparityMode.data());
	}
...
void UIEngine::glutKeyUpFunction(unsigned char key, int x, int y)
{
    ...
    case 'a':
        if(uiEngine->mainLoopAction == UIEngine::PROCESS_VIDEO)
            uiEngine->mainLoopAction = UIEngine::PROCESS_PAUSED;
        else uiEngine->mainLoopAction = UIEngine::PROCESS_VIDEO;
		break;
    ...
    case 'd': // change the compute disparity mathod
        computeDisparity->computeDisparityType++; 
        if(computeDisparity->computeDisparityType > SPS_MODE) 
            computeDisparity->computeDisparityType = FILE_MODE;
		break;
...
//                    (winReg[w][0], winReg[w][3]); // zuo shang
	...
	inputLeftRGBImage = new ITMUChar4Image(imageSource->getRGBImageSize(), true, allocateGPU);
    inputRightRGBImage = new ITMUChar4Image(imageSource->getRGBImageSize(), true, allocateGPU);
	...
	if (inputLeftRGBImage->noDims == Vector2i(0,0)) {
...
void UIEngine::Shutdown()
{
	...
	delete inputLeftRGBImage;
    delete inputRightRGBImage;
