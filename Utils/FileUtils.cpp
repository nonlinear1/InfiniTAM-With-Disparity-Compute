...
{  
  ...
  // TODO(andrei): Improve this error handling.
	...
	leftImage->ChangeDims(newSize);
	Vector4u *dataPtr = leftImage->GetData(MEMORYDEVICE_CPU);

	unsigned char *data;
	if (type != RGBA_8u) data = new unsigned char[xsize*ysize * 3];
	else data = (unsigned char*)leftImage->GetData(MEMORYDEVICE_CPU);
  ...
  // TODO(andrei): Improve error reporting here.
    ...
    if (type != RGBA_8u)
	{
		for (int i = 0; i < leftImage->noDims.x*leftImage->noDims.y; ++i)
		{  
    ...
    if(computeDisparity->computeDisparityType != FILE_MODE)
    {
        ...
        cvGetRawData(leftImageForCompute,  &computeDisparity->stereoImage.I1);
        cvGetRawData(rightImageForCompute, &computeDisparity->stereoImage.I2);
